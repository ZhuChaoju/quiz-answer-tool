...
from .text import PREFIX_RE, find_answer_line
...
def locate_icon(img: Image.Image, search: dict, panel_bg, tol: float = 28.0):
    ...
    for sy in range(hh):
        for sx in range(ww):
            ...
            if not (30 <= bw <= 55 and 30 <= bh <= 55):
                continue
            fill = n / (bw * bh)
            square = 1.0 - abs(bw - bh) / max(bw, bh)
            score = fill * 0.6 + square * 0.4
            if best is None or score > best[0]:
                best = (score, x0 + min_x, y0 + min_y, x0 + max_x + 1, y0 + max_y + 1)
...
class IconModule(BaseModule):
    ...
    def recognize(self, frame: Image.Image, ocr_cfg: dict[str, Any]) -> ModuleResult:
        ...
        o_lines = ocr.recognize(o_crop, lang, conf, mt, True, dml)
        res.lines = o_lines
